...
class AmazonBot:
    ...
    def get_product_rating(self, soup):
        try:
            div_avg_cust_reviews = soup.find(
                'div', {'id': 'averageCustomerReviews'})
            rating = div_avg_cust_reviews.find(
                'span', {'class': 'a-icon-alt'}).get_text().strip().split()[0]
    ...
    def get_product_nb_reviewers(self, soup):
        try:
            nb_reviewers = soup.find(
                'span', {'id': 'acrCustomerReviewText'}).get_text().strip()
        ...
        try:
            ...
            try:
                price = soup.find('span', {
                                  'class': 'a-size-medium a-color-price header-price a-text-normal'}).get_text().strip()
                return float(price.split()[0].replace(',', '.'))
            except:
                try:
                    price = soup.find('span', {
                                      'class': 'a-size-base a-color-price header-price a-text-normal'}).get_text().strip()
                    return float(price.split()[0].replace(',', '.'))
                except:
                    try:
                        price = soup.find('span', {'class': 'a-price-whole'}).get_text(
                        ) + soup.find('span', {'class': 'a-price-fraction'}).get_text()
                        return float(price.split()[0].replace(',', '.'))
                    except:
                        return None
        ...
        while True:
            ...
            product_urls = self.mongodb_client["amazon"]["product_urls"].find({
                "$or": [
                    ...
                    {"updated_at": {"$lte": datetime.datetime.now() - \
                ...
                # du prix que l'on vient de récuperer
                    ...
                    if (type(data["price"]) is int or type(data['price']) is float) and \
                        ...
                        diff_price_percentage = (
                            1 - data["price"] / last_product_price["price"]) * 100

                        # S'il y a une baisse du prix on envoie un email
                        if diff_price_percentage > 0:
                            message = """

                            Tout d'abord bonjour et j'espère que tu vas bien.
                            ...
                            SENDER_EMAIL_ADDRESS = "projet.webscraping.amazonw01@gmail.com"
                            RECIPIENT_EMAIL_ADDRESS = [
                                "projet.webscraping.amazonw01@gmail.com",
                                "mahamoudabdek@gmail.com",
                                "adensaidosman99@gmail.com",
                                "asmaabdihassan@gmail.com",
                                "abdoulazizzongo0@gmail.com",
                                "jsetondji@gmail.com",
                                "samadnureye@gmail.com",
                                "roki.youssouf@gmail.com",
                                "filan.aden@gmail.com"
                            ]
                            self.server_smtp.sendmail(
                                SENDER_EMAIL_ADDRESS, RECIPIENT_EMAIL_ADDRESS, message)
